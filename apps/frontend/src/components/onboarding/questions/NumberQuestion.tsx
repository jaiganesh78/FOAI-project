...
export const NumberQuestion: React.FC<Props> = ({ question, value, onChange }) => {
  return (
    <div className="space-y-2">
      ...
      <input
        ...
        placeholder={question.placeholder || 'e.g. 2,50,000 or 250000'}
        disabled={question.isDisabled}
        aria-label={question.renderingMetadata?.ariaLabel || question.label}
        onChange={(e) => onChange(e.target.value)}
        className="w-full rounded-lg border border-slate-700 bg-slate-900/80 px-4 py-2.5 text-sm text-slate-100 placeholder-slate-500 focus:border-cyan-500 focus:outline-none focus:ring-1 focus:ring-cyan-500 disabled:opacity-50"
      />
